...
public:
    ...
    Wrapper_1(const char * name){
      ...
      signalAccess[1] = new CDataSignalAccess(&(top.io_dataIn_valid));
      signalAccess[2] = new IDataSignalAccess(&(top.io_dataIn_payload));
      signalAccess[3] = new CDataSignalAccess(&(top.io_dataIn_ready));
      signalAccess[4] = new CDataSignalAccess(&(top.io_dataOut_valid));
      signalAccess[5] = new IDataSignalAccess(&(top.io_dataOut_payload));
      signalAccess[6] = new CDataSignalAccess(&(top.io_dataOut_ready));
      signalAccess[7] = new IDataSignalAccess(&(top.io_pcIn));
      signalAccess[8] = new IDataSignalAccess(&(top.io_pcOut));
      ...
      top.trace(&tfp, 99);
      tfp.open((std::string("D:\\Work\\Code\\ChiselPrj\\.\\simWorkspace\\FetchFifo/") + name + ".vcd").c_str());
      #endif
    }
