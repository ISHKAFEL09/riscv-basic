...
public:
    ...
    Wrapper_1(const char * name){
      time = 0;
      waveEnabled = true;
      signalAccess[0] = new CDataSignalAccess(&(top.io_opcode));
      signalAccess[1] = new IDataSignalAccess(&(top.io_op1));
      signalAccess[2] = new IDataSignalAccess(&(top.io_op2));
      signalAccess[3] = new IDataSignalAccess(&(top.io_aluOut));

      #ifdef TRACE
      Verilated::traceEverOn(true);
      top.trace(&tfp, 99);
      tfp.open((std::string("D:\\Work\\Code\\ChiselPrj\\.\\simWorkspace\\ALU/") + name + ".vcd").c_str());
      #endif
    }
